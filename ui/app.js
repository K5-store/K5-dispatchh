...
function openDISPATCH() {
	...
	if ($(`.dispatch-holder`).css("display") != "block" || !ControlMenu) {
		...
		if (GetCurrentPage() != undefined)
			$.post("https://K5-dispatch/enableRespond", JSON.stringify({ callId: GetCurrentPage().callId }));
		else
			$.post("https://K5-dispatch/enableRespond", JSON.stringify({ callId: undefined }));
		clearTimeout(timeOut);
		clearTimeout(timeOutLate);
		ControlMenu = true;
	} else {
		...
		$.post("https://K5-dispatch/disableDispatchMenu", JSON.stringify({ }));
